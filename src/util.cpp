...
const char * const BITCOIN_CONF_FILENAME = "escudonavacense.conf";
const char * const BITCOIN_PID_FILENAME = "escudonavacensed.pid";
...
#else
    const char* pszModule = "escudonavacense";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EscudoNavacense";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EscudoNavacense";
#else
    // Unix
    return pathRet / ".escudonavacense";
#endif
#endif
}
